...
class _ErkekState extends State<Erkek> {
  ...
  void initState() {
    ...
    _angimeQuery = _database.reference().child("Jatawka");
  ...
  onEntryAdded(Event event) {
    ...
    setState(() {
      _angimeListNew.add(Eerdb.fromSnapshot(event.snapshot));
  ...
  onEntryChanged(Event event) {
    ...
    setState(() {
      _angimeListNew[_angimeListNew.indexOf(oldEntry)] =
          Eerdb.fromSnapshot(event.snapshot);
    });
  }
  ...
  Widget bookList(BuildContext context, List<Eerdb> _angimeListNew) {
    if (_angimeListNew.length > 0) {
      return Container(
        child: angimeItem(context, eerdb),
      );
    } else {
  ...
  Widget angimeItem(BuildContext context,Eerdb eerdb) {
    Size size = MediaQuery.of(context).size;
    double height = MediaQuery.of(context).size.height;
    double width = MediaQuery.of(context).size.width;
    double text = MediaQuery.textScaleFactorOf(context);
